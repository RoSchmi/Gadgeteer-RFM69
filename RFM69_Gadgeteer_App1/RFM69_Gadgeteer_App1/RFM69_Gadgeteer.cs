...
{
    class RFM69_Gadgeteer : GTM.Module
    {
...
//***************** End of RFM69 Registers and Definitions ***************
        ...
        private GTI.Spi spi;
        private GTI.SpiConfiguration MySpiConfig;
        ...
        //private GTI.DigitalOutput trigger_P5; // Only needed for impulses for logic analyzer
		...
		/// <param name="socketNumber">The Gadgeteer socket that this module is plugged into.</param>
        ...
        public RFM69_Gadgeteer(int socketNumber, bool isRFM69HW, string pLabel = "", string pLocation = "", string pMeasuredQuantity = "", string pDestinationTable = "")
        {
            this.label = pLabel;
			...
			this.socket = GT.Socket.GetSocket(socketNumber, true, this, null);
			socket.EnsureTypeIsSupported('S', this);
			socket.ReservePin(GT.Socket.Pin.Three, this);  // Go, Interrupt
			socket.ReservePin(GT.Socket.Pin.Four, this);   // Reset
			socket.ReservePin(GT.Socket.Pin.Six, this);    // Select
            ...
            this.MySpiConfig = new GTI.SpiConfiguration(SPI_CS_ACTIVE_HIGH, SPI_CS_SETUP_TIME, 
                                                        SPI_CS_HOLD_TIME, SPI_CLOCK_IDLE_HIGH, 
                                                        SPI_SAMPLING_EDGE_RISING, SPI_CLOCK_RATE_KHZ);

            this.spi = GTI.SpiFactory.Create(socket, MySpiConfig, GTI.SpiSharing.Shared, socket, GT.Socket.Pin.Six, this);

            this.input = new InterruptPort(socket.CpuPins[3], false, Port.ResistorMode.PullUp, Port.InterruptMode.InterruptEdgeHigh);
            ...
            this.reset = GTI.DigitalOutputFactory.Create(socket, GT.Socket.Pin.Four, false, this);
            //this.trigger_P5 = GTI.DigitalOutputFactory.Create(socket, GT.Socket.Pin.Five, true, this);  // optional: used to trigger logicanalyzer
		}
        #endregion
        ...
        void runReceiveLoop()
        {
            ...
            while (true)
            {
                ...
                //Thread.Sleep(0);    // ToDo find out if needed and if at the rigth place
                /*
                lock(LockReceiveDone)
                { 
                    receiveDone("RunRec_2"); //put radio in RX mode
                }
                */
                Thread.Sleep(10);
        ...
        public delegate void ReceiveLoopEventHandler(RFM69_Gadgeteer sender, ReceiveLoopEventArgs e);
        /// <summary>
        ...
        private void OnReceiveLoopIteration(RFM69_Gadgeteer sender, ReceiveLoopEventArgs e)
        {
            if (this.onReceiveLoopIteration == null)
            {
                this.onReceiveLoopIteration = this.OnReceiveLoopIteration;
            }
            if (GT.Program.CheckAndInvoke(this.ReceiveLoopIteration, this.onReceiveLoopIteration, sender, e))
        ...
        public delegate void ACKReturnedEventHandler(RFM69_Gadgeteer sender, ACK_EventArgs e);
        ...
        private void OnACKReturned(RFM69_Gadgeteer sender, ACK_EventArgs e)
        {
            if (this.onACKReturned == null)
            {
                this.onACKReturned = this.OnACKReturned;
            }
            if (GT.Program.CheckAndInvoke(this.ACKReturned, this.onACKReturned, sender, e))
        ...
        /// <param name="sender">The <see cref="RFM69_Device"/> object that raised the event.</param>
        /// <param name="e">The event arguments.</param>
        public delegate void MessageReceivedEventHandler(RFM69_Gadgeteer sender, MessageReceivedEventArgs e);
        ...
        private void OnMessageReceived(RFM69_Gadgeteer sender, MessageReceivedEventArgs e)
        {
